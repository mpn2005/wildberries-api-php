...
Метод позволяет устанавливать/удалять минус-фразы точного соответствия. <span class=\"newM\">new</span><br> Отправка пустого массива удаляет все минус-фразы точного соответствия из кампании.
...
Метод позволяет получать историю затрат. <span class=\"newM\">new</span>
...
Метод позволяет получать массив с датами начала месяца, когда у продавца были расходы.  <span class=\"newM\">new</span>
...
Метод позволяет получить отзыв по его Id. <span class=\"newM\">new</span>
...
Метод позволяет получить количество отзывов. <span class=\"newM\">new</span>
...
Метод позволяет получить список рекомендаций по нескольким товарам. <span class=\"newM\">new</span><br> <br> `ВАЖНО`: Если Вы получили ошибку с кодом 200 и сообщением вида <b>CrutchSapi1356-65c49a25-7f9798142620-UKfmVPyPcik6d OK</b>, значит минимум одна из переданных номенклатур не принадлежит Вам, либо не существует.
...
Метод позволяет получить вопрос по его Id. <span class=\"newM\">new</span><br>
...
Метод позволяет получить количество вопросов. <span class=\"newM\">new</span>
...
Метод позволяет удалить шаблон. <span class=\"newM\">new</span><br>
...
Метод позволяет получить шаблоны ответов на отзывы/вопросы. <span class=\"newM\">new</span>
...
Метод позволяет отредактировать шаблон. <span class=\"newM\">new</span><br> Допустимы любые символы.
...
Метод позволяет создать шаблон ответа на отзыв/вопрос. <span class=\"newM\">new</span><br> Всего можно создать 20 шаблонов. 10 на отзывы и 10 на вопросы.<br> Допустимы любые символы.
...
Метод позволяет объединять номенклатуры (`nmID`) под одним `imtID` и разъединять их.<br> Для объединения НМ необходимо отправить запрос со списком НМ с заполненным параметром `targetIMT` в теле запроса.  При этом все НМ объединятся под `imtID` указанном в `targetIMT`. <br> <br> Чтобы отсоединить НМ от карточки, необходимо передать эту НМ без параметра `targetIMT` в теле запроса.<br> При этом для передаваемой НМ создается новый `imtID`. <br> Если в запросе передается несколько НМ, то все они автоматически объединятся под новым `imtID`. <br> Для НМ, не участвующих в запросе, никаких изменений не происходит.<br> <br> `ВАЖНО:` Объединить можно только номенклатуры с одинаковыми предметами.<br> `ВАЖНО:` В одной КТ (под одним `imtID`) не может быть больше 30 номенклатур (`nmID`).
